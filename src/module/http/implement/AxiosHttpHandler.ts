...
class AxiosHttpHandler implements HttpHandler {
  ...
  async post<T, D = object>(url: string, data: D, config?: AxiosRequestConfigAdaptor) {
  ...
  private getRequestConfig(url: string, config?: AxiosRequestConfigAdaptor): AxiosRequestConfig {
    if (!config) return {}
    const defaultConfig: AxiosRequestConfig = {
      headers: getRequestHeader(requestHeaderMap, extractDomain(url), config.token),
      /**
